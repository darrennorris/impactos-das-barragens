# prep vector
# produce zones around area diretamente afetada. 
# 
# calculate buffers: 1000, 2000, 4000, 8000, 16000 
...
# 2km
ad_buf_2km <- st_buffer(ad, dist=2000) %>% 
...
#
bind_rows(ad_buf_1km, dif_buf_1km, 
          ad_buf_2km, dif_buf_2km, 
          ad_buf_4km, dif_buf_4km, 
          ad_buf_8km, dif_buf_8km) %>% 
  ggplot() +
  geom_sf(aes(fill=area_km2)) + 
  geom_sf(data = ad_line) +
...
st_write(ad_dif, dsn = outfile, 
         layer = "faixas", delete_layer = TRUE, append = TRUE)
